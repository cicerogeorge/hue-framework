...
class Form
{
    ...
    function Input($params=array(), $model=FALSE, $view=FALSE, $required=FALSE)
    {
        if (is_array($params) && count($params))
        {
            ...
            if (!$name && $model && $view)
            {
                $Input .= " name=\"form[".ucwords($model)."][".$view."]"."\"";
                if ($params['type'] != "password" && !array_search('value', $keys))
                {
                    $Input .= " value=\"".@$_POST['form'][ucwords($model)][$view]."\"";
                }
            }
            if (!$id && $model && $view)
            {
            ...
            if ($required)
            {
                $Input .= "&nbsp;<i class=\"icon-asterisk mdt\"></i>";
            }
            echo $Input;
        }
        return FALSE;
    ...
    function Select($collection, $params=array(), $model=FALSE, $view=FALSE, $inner_html=FALSE, $check_vlw=FALSE, $mdt=FALSE, $vlw='id')
    {
        $Select = "<select";
        ...
        {
            $Select .= " name=\"form[".ucwords($model)."][".$view."]\"";
        }
        $Select .= ">";
        ...
        if (is_array($collection))
        {
            ...
            {
                $check_vlw = $check_vlw ? $check_vlw : @$_POST['form'][ucwords($model)][$view];
                // if (is_object($c))// array de objetos
                // {
                    $select = ($check_vlw == $c[$vlw]) && $c[$vlw] ? " selected=\"selected\"" : "";
                    $Select .= "<option value=\"".$c[$vlw]."\"$select>".$c[$inner_html]."</option>\n";
                // }
                // else
                // {
        ...
        if($mdt)
        {
            //$Select .= "&nbsp;<i class=\"icon-asterisk mdt\"></i>";
        }
        echo $Select;
        return FALSE;
    }
    /* Generates textarea fields */
    function Textarea($params, $model, $attr, $innerHTML='')
    {
        if (is_array($params))
        {
            $Textarea = "<textarea name=\"form[".ucwords($model)."][".$attr."]\" id=\"".ucwords($model)."_".ucwords($attr)."\"";
            foreach ($params as $k=>$v)
            {
                $Textarea .= " ".$k."=\"".$v."\"";
            }
			if ($_POST || $innerHTML=='')
			{
				$innerHTML = @$_POST['form'][ucwords($model)][$attr];
			}
